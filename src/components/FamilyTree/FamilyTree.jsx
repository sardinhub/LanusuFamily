...
// Shared D3 tree renderer — dipakai di mode normal & fullscreen
function renderTree({ svgRef, silsilah, collapsed, toggleCollapse, setSelectedMember }) {
  ...
  // Store zoom behavior on svg element for external control
  const zoom = d3.zoom().scaleExtent([0.15, 4]).on("zoom", (event) => {
    g.attr("transform", event.transform);
  });
  svg.call(zoom);
  // Expose zoom + initial transform for control buttons
  ...
  node.select("rect")
    .on("mouseenter", function () {
      d3.select(this).transition().duration(150).attr("stroke-opacity", 0.9).attr("stroke-width", 2);
    })
    .on("mouseleave", function () {
      d3.select(this).transition().duration(150).attr("stroke-opacity", 0.45).attr("stroke-width", 1.2);
    });

  // Gender icon circle
  node.append("circle")
    .attr("cx", -NODE_W / 2 + 22).attr("cy", 0).attr("r", 16)
    .attr("fill", (d) => BRANCH[d.data.cabang]?.dim || "rgba(255,255,255,0.05)")
    .attr("stroke", (d) => BRANCH[d.data.cabang]?.color || "#4b5563")
    .attr("stroke-opacity", 0.4).attr("stroke-width", 1);

  node.append("text")
    .attr("x", -NODE_W / 2 + 22).attr("y", 5)
    .attr("text-anchor", "middle").attr("font-size", 14)
    .text((d) => (d.data.jenis_kelamin === "L" ? "👨" : "👩"));

  // Name
  node.append("text")
    .attr("x", -NODE_W / 2 + 46).attr("y", -8)
  ...
  node.filter((d) => d.data._hasChildren)
    ...
    .call((g) => {
      g.append("circle")
        ...
        .attr("stroke-opacity", 0.6).attr("stroke-width", 1.2);
      g.append("text")
  ...
  // Info button
  node.append("text")
    .attr("x", NODE_W / 2 - 14).attr("y", -NODE_H / 2 + 16)
...
export default function FamilyTree() {
  ...
  useEffect(() => {
    if (!silsilah) return;
    renderTree({ svgRef, silsilah, collapsed, toggleCollapse, setSelectedMember });
  }, [collapsed, dimensions, setSelectedMember, toggleCollapse, silsilah, fullscreen]);

  // Zoom control helpers
  const handleZoom = useCallback((factor) => {
    const svgEl = d3.select(svgRef.current);
    const zb = svgRef.current?.__zoom_behavior__;
    if (!zb) return;
    svgEl.transition().duration(300).call(zb.scaleBy, factor);
  }, []);
  ...
  const handleZoomReset = useCallback(() => {
    ...
    const zb = svgRef.current?.__zoom_behavior__;
    if (!zb) return;
    svgEl.transition().duration(400).call(zb.transform, d3.zoomIdentity);
  }, []);
  ...
  const Legend = () => (
    <div className="family-tree-legend">
      ...
      <span className="legend-tip">💡 Klik nama untuk detail · Klik +/− untuk buka/tutup · Scroll/cubit untuk zoom</span>
    </div>
  );

  const ZoomControls = () => (
    <div className="tree-zoom-controls">
      <button className="tree-zoom-btn" onClick={() => handleZoom(1.4)} title="Perbesar">＋</button>
      <button className="tree-zoom-btn" onClick={() => handleZoom(0.7)} title="Perkecil">－</button>
      <button className="tree-zoom-btn" onClick={handleZoomReset} title="Reset zoom">⌂</button>
      <button
        className="tree-zoom-btn tree-zoom-btn--fullscreen"
        onClick={() => setFullscreen((v) => !v)}
        title={fullscreen ? "Tutup layar penuh (Esc)" : "Layar penuh"}
      >
        {fullscreen ? "✕" : "⛶"}
      </button>
    </div>
    ...
    return (
      <div className="family-tree-fullscreen">
        ...
        <div className="tree-fullscreen-header">
          <div className="tree-fullscreen-title">
            <span>🌳</span>
            <span>Silsilah Keluarga La Nusu</span>
          </div>
          <Legend />
          <ZoomControls />
        </div>
        {/* SVG canvas */}
        <svg ref={svgRef} className="family-tree-svg family-tree-svg--fullscreen" />
      </div>
    );
  }
